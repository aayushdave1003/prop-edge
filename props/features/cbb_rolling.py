"""Compute rolling features for CBB (men's college basketball) player-games.

Identical logic to nba_rolling/wnba_rolling — filters on sport_code='cbb'. Produces
the core rolling features (last_5/10/20/season avgs of the box-score stats + the
combo sums + threshold rates) that the CBB player models consume. Self-contained:
no dependency on the advanced/IQ feature modules.
"""
...
def compute_all(df: pd.DataFrame) -> pd.DataFrame:
    log.info("computing_cbb_rolling_features")
...
def write_to_derived(feature_df: pd.DataFrame):
    ...
    write_derived(items, mode="replace", label="cbb_rolling")
...
def run():
    ...
    with session_scope() as session:
        run_id = session.execute(text("""
            ...
            VALUES ('cbb_rolling', :s, 'running') RETURNING run_id
        """), {"s": started}).scalar()

    df = load_player_games()
    if df.empty:
        log.info("no_cbb_games_yet")
        return
    df  = explode_stats(df)
    out = compute_all(df)
    ...
    log.info("cbb_rolling_complete", updated=len(out))
