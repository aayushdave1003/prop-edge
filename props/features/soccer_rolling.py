"""Rolling features for soccer player-games (sport_code='soccer'). ~38 games/
season, so short windows (last 3/5 + season). Covers the priceable markets:
shots, shots-on-target, fouls, saves, goals, assists."""
...
WINDOWS = [3, 5]
ROLL_STATS = ["shots", "shots_on_target", "fouls", "fouls_drawn",
              "saves", "goals", "assists", "yellow_cards"]


def load_player_games() -> pd.DataFrame:
    log.info("loading_soccer_player_games")
    df = pd.read_sql("""
...
def compute_rolling_for_player(group: pd.DataFrame) -> pd.DataFrame:
    ...
    sm = (g["season"] != g["season"].shift(1)).cumsum()
    feats["games_played_season"] = g.groupby(sm).cumcount().values
    for stat in ROLL_STATS:
        pv = g[stat].shift(1)
        for w in WINDOWS:
            feats[f"last_{w}_avg_{stat}"] = pv.rolling(w, min_periods=1).mean().fillna(0).round(3).values
        cum = g.groupby(sm)[stat].apply(lambda s: s.shift(1).expanding().mean()).reset_index(level=0, drop=True).fillna(0)
...
def run():
    ...
    with session_scope() as session:
        run_id = session.execute(text("""
            ...
            VALUES ('soccer_rolling', :s, 'running') RETURNING run_id
        """), {"s": started}).scalar()
    df = load_player_games()
    if df.empty:
        log.info("no_soccer_games_yet")
        return
    df = explode_stats(df)
    ...
    write_derived([(int(r["player_game_id"]), feat_dict(r, cols)) for _, r in out.iterrows()],
                  mode="replace", label="soccer_rolling")
    with session_scope() as session:
    ...
    log.info("soccer_rolling_complete", updated=len(out))
