"""Train the hits+runs+rbis combo model (PrizePicks' #1 MLB market).

Rather than summing three component models (and runs alone is barely better than
its season mean), this fits ONE Poisson model directly on the combo target
y = hits + runs + rbis, drawing on hitting-volume, power, on-base, lineup-spot
and opposing-pitcher features. Filter: PA >= 3.
"""
import json
...
MODEL_PATH = MODEL_DIR / "hits_runs_rbis_v1.txt"
META_PATH  = MODEL_DIR / "hits_runs_rbis_v1_meta.json"

FEATURE_KEYS = [
    "last_5_avg_hits","last_10_avg_hits","last_20_avg_hits","season_avg_hits",
    "last_5_avg_runs","last_10_avg_runs","season_avg_runs",
    "last_5_avg_rbis","last_10_avg_rbis","last_20_avg_rbis","season_avg_rbis",
    "last_5_avg_total_bases","last_10_avg_total_bases",
    "last_5_avg_home_runs","last_10_avg_home_runs",
    "last_5_avg_walks","last_10_avg_walks",
    "last_5_avg_at_bats","last_10_avg_at_bats",
    "last_5_avg_bat_order_spot","last_10_avg_bat_order_spot","season_avg_bat_order_spot",
    "last_10_avg_batter_slg","last_10_avg_batter_iso","last_10_avg_batter_hard_contact",
    ...
    "last_10_avg_pitcher_command","park_factor",
]
...
def load_training_data():
    ...
    y = sum(pd.to_numeric(stats[s], errors="coerce").fillna(0) for s in ("hits", "runs", "rbis"))
    out = pd.DataFrame({
    ...
    # combined season-mean baseline for the MAE comparison
    out["baseline"] = sum(out[f"season_avg_{s}"] for s in ("hits", "runs", "rbis"))
...
def evaluate(model, test_df):
    ...
    y    = test_df["y"].values
    baseline = test_df["baseline"].values
...
def main():
    ...
    meta = {"model_path":str(MODEL_PATH),"target":"hits_runs_rbis","feature_keys":FEATURE_KEYS,
