"""Train NHL saves prediction model (goalies only)."""
import json
...
MODEL_PATH = MODEL_DIR / "nhl_saves_v1.txt"
META_PATH  = MODEL_DIR / "nhl_saves_v1_meta.json"
...
FEATURE_KEYS = [
    ...
    "last_20_avg_saves",
    "season_avg_saves",
    "last_10_rate_over_19.5_saves",
    "last_10_rate_over_24.5_saves",
    "last_10_rate_over_29.5_saves",
    "last_5_avg_shots_against",
    "last_10_avg_shots_against",
    "season_avg_shots_against",
    "last_5_avg_goalie_save_pct",
    "last_10_avg_goalie_save_pct",
    "last_5_avg_goalie_workload",
    "last_10_avg_goalie_workload",
    "last_5_avg_goals_against",
...
def load_training_data():
    log.info("loading_nhl_goalie_training_data")
    df = pd.read_sql("""
    ...
    out = out[out["last_10_avg_saves"] > 0].copy()
...
def train_model(train_df, val_df):
    ...
    params = {
        ...
        "learning_rate": 0.05, "num_leaves": 10, "min_data_in_leaf": 5,
...
def main():
    ...
    if len(df) < 8:
        log.warning("insufficient_goalie_data", n=len(df))
        return
    ...
    if len(test_df) > 0:
        ...
        mae_b  = np.mean(np.abs(test_df["season_avg_saves"].values - y_test))
