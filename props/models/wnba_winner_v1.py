"""Train WNBA game winner model: win probability + implied point spread.

Two LightGBM models trained on ~327 completed WNBA games (2025 season; test on 2026):
...
CLF_PATH   = MODEL_DIR / "wnba_winner_v1_classifier.txt"
REG_PATH   = MODEL_DIR / "wnba_winner_v1_regressor.txt"
META_PATH  = MODEL_DIR / "wnba_winner_v1_meta.json"
...
def main():
    ...
    # Random val holdout for early stopping: the 2025 WNBA season is short, so a
    # time-tail val lands on ~14 end-of-season games and stops training at iter 1.
    # A 15% random val (~47 games) gives stable early stopping; the TEST set stays
    # a clean time-based 2026 holdout.
    val_df  = train_all.sample(frac=0.15, random_state=42)
    fit_df  = train_all.drop(val_df.index)
