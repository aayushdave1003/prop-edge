"""Train WNBA assists prediction model."""
import json
...
MODEL_PATH = MODEL_DIR / "wnba_assists_v1.txt"
META_PATH  = MODEL_DIR / "wnba_assists_v1_meta.json"
...
def load_training_data():
    ...
    df = pd.read_sql("""
        ...
        WHERE g.sport_code = 'wnba' AND pg.minutes_played >= 5
    """, engine)
...
def main():
    ...
    y_test = pd.to_numeric(test_df["y"], errors="coerce").values
    pred   = model.predict(test_df[FEATURE_KEYS], num_iteration=model.best_iteration)
    mae_m  = np.mean(np.abs(pred - y_test))
    mae_b  = np.mean(np.abs(test_df["season_avg_assists"].values - y_test))
