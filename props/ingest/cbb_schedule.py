"""Ingest men's college basketball (CBB) schedule into the games table via ESPN.

Mirrors wnba_schedule — same ESPN scoreboard shape, sport_code='cbb'. CBB slates
are large (100+ games/day in season), so request a high limit + all groups.
"""
import requests
...
def _normalize_status(detail: str) -> str:
    ...
    if "postponed" in d or "canceled" in d:
        return "postponed"
    if any(q in d for q in ["1st", "2nd", "half", "overtime", "ot"]):
        return "live"
    return "scheduled"


def fetch_cbb_games(target_date: date) -> list[dict]:
    r = requests.get(ESPN_SCOREBOARD,
                     params={"dates": target_date.strftime("%Y%m%d"), "groups": "50", "limit": "400"},
                     timeout=20)
    r.raise_for_status()
    events = r.json().get("events", [])
    log.info("fetched_cbb_games", date=target_date.isoformat(), count=len(events))
...
def upsert_games(games: list[dict], target_date: date):
    ...
    with session_scope() as session:
            ...
            for side in [("home_team_ext", "home_team_name", "home_team_abbr"),
                ...
                if ext:
                    session.execute(text("""
                        ...
                        VALUES ('cbb', :ext, :name, :abbr)
                        ON CONFLICT (sport_code, external_id) DO UPDATE
                        SET name = EXCLUDED.name
                    """), {"ext": ext, "name": name, "abbr": abbr[:5] if abbr else name[:5]})

        tid_map = {r[0]: r[1] for r in session.execute(text(
            "SELECT external_id, team_id FROM teams WHERE sport_code='cbb'")).all()}

        inserted = updated = 0
        for g in games:
            home_tid, away_tid = tid_map.get(g["home_team_ext"]), tid_map.get(g["away_team_ext"])
            ...
            result = session.execute(text("""
                ...
                VALUES ('cbb', :ext, :d, :dt, :season, 'regular',
        ...
        log.info("cbb_schedule_ingest_complete", inserted=inserted, updated=updated)
...
def run(target_date: date = None):
    ...
    log.info("fetching_cbb_schedule", date=target_date.isoformat())
    upsert_games(fetch_cbb_games(target_date), target_date)
