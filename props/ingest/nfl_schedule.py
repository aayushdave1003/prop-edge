"""Ingest NFL schedule into the games table via ESPN.

Mirrors cbb_schedule — ESPN scoreboard, sport_code='nfl'. NFL is 32 teams, ~16
games/week (Thu/Sun/Mon), season Sep–Jan. Date-based fetch (most dates have 0).
"""
import requests
...
def _normalize_status(detail: str) -> str:
    ...
    if "postponed" in d or "canceled" in d:
        return "postponed"
    if any(q in d for q in ["1st", "2nd", "3rd", "4th", "half", "overtime", "ot", "in progress"]):
        return "live"
    return "scheduled"


def fetch_nfl_games(target_date: date) -> list[dict]:
    r = requests.get(ESPN_SCOREBOARD,
                     params={"dates": target_date.strftime("%Y%m%d")}, timeout=20)
    r.raise_for_status()
    events = r.json().get("events", [])
    log.info("fetched_nfl_games", date=target_date.isoformat(), count=len(events))
    out = []
    for ev in events:
        ...
        out.append({
            "external_id": ev["id"], "game_datetime": comp.get("startDate"),
...
def upsert_games(games: list[dict], target_date: date):
    # NFL season spans two calendar years (Sep–Jan); ESPN labels by start year.
    ...
    with session_scope() as session:
        ...
        tid = {r[0]: r[1] for r in session.execute(text(
            "SELECT external_id, team_id FROM teams WHERE sport_code='nfl'")).all()}
        inserted = updated = 0
        for g in games:
            ...
            res = session.execute(text("""
                ...
                VALUES ('nfl', :ext, :d, :dt, :season, 'regular', :h, :a, :hs, :as_, :status)
        ...
        log.info("nfl_schedule_ingest_complete", inserted=inserted, updated=updated)
...
def run(target_date: date = None):
    ...
    log.info("fetching_nfl_schedule", date=target_date.isoformat())
    upsert_games(fetch_nfl_games(target_date), target_date)
