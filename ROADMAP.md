...
## 2. Model / analytics
- ☐ **P2** **The 2025 data gap poisons current-season training — the split can't just "roll forward."** `hits_v1` et al. hardcode a `2025-01-01` boundary, so the production models train on ~6 weeks of 2024 (Aug 15–Sep 28) and never on 2026 (no 2025 data exists). Intuitively "roll the split forward so it learns 2026" — but I **built and tested** that (a shared rolling-split util, since reverted) and it **consistently HURT** held-out 2026 MAE: 2024-only+time-val **0.6780** vs every rolling config **0.685–0.687** (5 configs, n=3,454 held-out). Cause: with no 2025 data, an early-2026 game's `last_10_avg_*` rolls back ~18 months to Sept 2024, so those training rows carry garbage "form" features; the clean contiguous 2024 block is the better signal. Real fixes, in order: (a) **backfill 2025 history** (closes the gap — best), (b) gate rolling features on within-season continuity (drop rows whose last-10 spans a >Nd gap), or (c) revisit the rolling split mid-2026 once players have gap-free in-season history. Until then **2024-only is correct — leave it**. (Pilot-first discipline caught this before it shipped across 9 models.)
