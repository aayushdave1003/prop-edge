...
## 2. Model / analytics
- ☐ **P3** **Opponent-adjusted features** — BUILT + A/B'd, parked pending data. `mlb_batter_sos` rolls faced-pitcher quality (`last_10_avg_faced_era/k_rate`) with a skew-verified inference mirror — all committed. `props.features.sos_eval` tests it risk-free (recomputes the exact backfill values in-memory on prod data, no writes). Verdict: total_bases **rejected** (−0.3% MAE, 0% gain); hits **+0.41% mean across 7 seeds** (7/7 positive, 15.6% gain share) — a real signal but **under the 0.5% promote bar**. The backfill is NOT the blocker (a 17k-row single-key merge ≈18 MB churn on a 460 MB DB w/ active autovacuum — small); the marginal sub-bar gain is. Re-run `sos_eval` as the pre-2025 train set (~6k rows now) grows; ship hits-only once it clears the bar cleanly.
