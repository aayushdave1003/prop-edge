...
- ◧ **P3** **New sports** — **CBB foundation built, models weak.** Full 2025-26 men's college-basketball season ingested via ESPN (`cbb_schedule`/`cbb_boxscores`, 6,222 games / 194k player_games / 10k players), `cbb_rolling` features, `cbb_models_v1` trainer — all reusable. But CBB player props model POORLY: best rebs_asts +1.4% / pts_rebs_asts +1.35%, and points/assists/threes LOSE to the season-avg baseline (vs NBA +7% / WNBA +26%). Verified intrinsic, not a bug — a 24-ppg starter swings 13/43/17 night to night, so neither model nor mean predicts games. Models NOT activated (off-season + marginal). REVISIT November with the live season + a bettable-player-only focus, or accept CBB isn't a fit. **Soccer**: World Cup is un-modelable (1-3 games/player); the real target is club leagues (EPL etc.) — build for the August season. Other sports (tennis/golf/UFC) need new non-box-score architectures.
